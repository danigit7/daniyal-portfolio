export const portfolioData = [
  {
    _id: "6432bd33fd88a2d9e3288e48",
    aboutText: `Hello, I’m Daniyal!
I’m a passionate Full Stack Developer with hands-on experience in building modern, responsive web applications using the MERN stack (MongoDB, Express, React, Node.js). I love solving real-world problems with clean code and creative design. Whether it's frontend elegance or backend logic, I aim to deliver efficient, user-friendly digital experiences.`,
    section: "about",
    aboutImage:
      "/assets/daniyal.jpeg",
    aboutHoverImage:
      "https://res.cloudinary.com/zaeem-cloudinary-cloud/image/upload/v1728669539/hq7englaahhkffsuoc3c.jpg",
  },
  {
    _id: "6432c66de600088d794f1e91",
    section: "hero",
    typewriter: [
      "Hey this is Daniyal!",
      "fullStackDeveloper.tsx",
    ],
    title: "Full Stack Developer",
    heroImage: "https://static.vecteezy.com/system/resources/previews/016/017/020/non_2x/transparent-coding-development-icon-free-png.png",
  },
  {
    _id: "6432c881e600088d794f1e94",
    section: "certifications",
    certData: [
      {
        name: "Foundations Of Data Science",
        link: "https://www.coursera.org/account/accomplishments/verify/SFJTBS8RHRKJ",
        provider: "Google",
        offeredBy: "Coursera",
        logo: "/google.png",
        status: "Completed",
        learnPoints: ["Machine Learning", "Data Science", "Data Analysis"],
      },
    ],
  },
  {
    _id: "6432cb77e600088d794f1e96",
    section: "skills",
    skillData: [
      // {
      //   skillName: "Langchain",
      //   proficiency: "60",
      //   imageLink:
      //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRPHPngdPMFt39z2fpwOZISE8anEJ_WZhZL_w&s",
      // },
      // {
      //   skillName: "Pinecone database",
      //   proficiency: "60",
      //   imageLink:
      //     "data:image/jpeg;base64,",
      // },
      {
        skillName: "next.js",
        proficiency: "70",
        imageLink: "https://i.ibb.co/3vDj7JL/Next-js-900x0.png",
      },
      {
        skillName: "Figma",
        proficiency: "70",
        imageLink: "/figma.png",
      },
      // {
      //   skillName: "React native",
      //   proficiency: "60",
      //   imageLink:
      //     "data:image/png;base64,",
      // },
      {
        skillName: "TypeScript",
        proficiency: "60",
        imageLink: "https://i.ibb.co/V3Fw2Ft/Typescript-logo-2020-svg.png",
      },
      // {
      //   skillName: "Docker",
      //   proficiency: "60",
      //   imageLink:
      //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQLQKd_MRed_mZQlgrzQuUXVA3P39ssOVX8_g&s",
      // },
      {
        skillName: "python",
        proficiency: "60",
        imageLink: "https://i.ibb.co/mBKKN4S/python-logo-4k-i6-1600x1200.jpg",
      },
      {
        skillName: "Tailwind",
        proficiency: "90",
        imageLink: "https://i.ibb.co/mD3rstb/z8hzeszc9eb3sp3vp3qc.jpg",
      },
      {
        skillName: "node.js",
        proficiency: "60",
        imageLink: "https://i.ibb.co/rQS8r4P/node-js.png",
      },
      {
        skillName: "express.js",
        proficiency: "60",
        imageLink: "https://i.ibb.co/MBHgZ74/expressjs-logo-icon-169185.png",
      },
      {
        skillName: "react",
        proficiency: "70",
        imageLink: "https://i.ibb.co/YyGmCM0/react.png",
      },
      {
        skillName: "github",
        proficiency: "70",
        imageLink:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        skillName: "mongodb",
        proficiency: "60",
        imageLink:
          "https://cdn.worldvectorlogo.com/logos/mongodb-icon-1.svg",
      },
      {
        skillName: "javascript",
        proficiency: "80",
        imageLink:
          "https://upload.wikimedia.org/wikipedia/commons/6/6a/JavaScript-logo.png",
      }, 
      {
        skillName: "My Sql",
        proficiency: "60",
        imageLink:
          "https://cdn-icons-png.flaticon.com/512/5968/5968313.png",
      },
      {
        skillName: "firebase",
        proficiency: "60",
        imageLink:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTuawUuQCcq6fD-KpdmL4QixUOyqQqdrVNIDg&s",
      },
      // {
      //   skillName: "app write",
      //   proficiency: "60",
      //   imageLink:
      //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSfBzv23AtO1CUKO1jwTMzMFFKYH9vAr-vpbh8dws_7WJT9ioHVgrfiMnxoIRFa-zYpONI&usqp=CAU",
      // },
      // {
      //   skillName: "cloudflare",
      //   proficiency: "60",
      //   imageLink:
      //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTvN7Jw6tPshCPpd6Uq99BoSpf7xihtvpPeA&s",
      // },
      {
        skillName: "HTML",
        proficiency: "90",
        imageLink: "https://i.ibb.co/ZXpZGFG/Screenshot-2023-07-07-223352.png",
      },
      {
        skillName: "CSS",
        proficiency: "70",
        imageLink: "https://i.ibb.co/tq84XMc/css.png",
      },
    ],
  },
  {
    _id: "6432d818e600088d794f1e97",
    section: "projects",
    projectsData: [
       {
        projectName: "Hilton Club's Website",
        projectDesc:
          "Built The Hilton Club website using React JS and Firebase, featuring a modern UI, smooth navigation, and dynamic content management. Integrated WhatsApp messaging for direct customer inquiries and streamlined event booking communication.",
        projectImage:
          "/hilton-logo-blue.jpg",
        projectLink: "https://hilton-club.com",
        gitHubLink: "May 2023 - July 2023",
        showGithubRepo: false
      },
       {
        projectName: "Inzaghi",
        projectDesc:
          "Inzaghi is an AI-powered food recommendation chatbot built for Peshawar that helps users discover restaurants and dishes based on their budget and preferences. It provides quick, relevant suggestions by analyzing location, food type, and price range, making it easier to decide what to eat and where without wasting time scrolling through apps.",
        projectImage:
          "/inzaghi.jpg",
        projectLink: "https://huggingface.co/spaces/danida7/Inzaghi-FoodieAi",
        gitHubLink: "May 2023 - July 2023",
        showGithubRepo: false
      },
      {
        projectName: "Blood Bank Management System",
        projectDesc:
          "Developed a Blood Bank Management System where users can register as blood donors and easily find other donors based on blood type and location. The system also includes a compatibility chart that shows which blood groups can donate or receive from others, making the donation process informed and efficient.",
        projectImage:
          "/bloodBank-logo.png",
        projectLink: "https://bloodbankapplicationn.netlify.app/",
        gitHubLink: "May 2023 - July 2023",
        showGithubRepo: false
      },
      // {
      //   projectName: "Bot M13A",
      //   projectDesc:
      //     "BotM13A: Your AI Companion for M13A Engine Insights—Experience the future of automotive assistance with an AI-powered chatbot delivering expert knowledge on the M13A engine using Retrieval-Augmented Generation.",
      //   projectImage:
      //     "https://media.istockphoto.com/id/1010001882/vector/%C3%B0%C3%B0%C2%B5%C3%B1%C3%B0%C3%B1%C3%B1.jpg?s=612x612&w=0&k=20&c=1jeAr9KSx3sG7SKxUPR_j8WPSZq_NIKL0P-MA4F1xRw=",
      //   projectLink: "https://botm13a.vercel.app/",
      //   gitHubLink: "May 2023 - July 2023",
      //   showGithubRepo: false
      // },
      // {
      //   projectName: "Encrypted Credentials",
      //   projectDesc:
      //     "A web app designed to securely store your digital identities using advanced encryption and hashing techniques.",
      //   projectImage:
      //     "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQa6yE2bjHEJq81Lik0GnlOI4N5LvsdiSSwg&s",
      //   projectLink: "https://encrypted-credentials.vercel.app/",
      //   gitHubLink: "https://google.com",
      //   showGithubRepo: false
        
      // }
    ],
  },
  {
    _id: "6432d9f6e600088d794f1e98",
    section: "header",
    
    linkedin: "https://www.linkedin.com/in/daniyal-asim-731b17220/",
    show_linkedin: true,

    twitter: "https://x.com/danida7_",
    show_twitter: false,

    github: "https://github.com/danigit7",
    show_github: true,

    instagram: "https://www.instagram.com",
    show_instagram: true,

    resume: "https://resume.sunilband.me",
    show_resume: false,

    whatsapp: "https://wa.me/+923459030321",
    show_whatsapp: true,
  },
];
